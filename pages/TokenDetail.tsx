...
export default function TokenDetail() {
  ...
  if (!name || !symbol) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4 font-mono">
      ...
      <p className="text-green-500 animate-pulse">正在获取测试网代币数据...</p>
    </div>
  );
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card-bg pump-border rounded-2xl p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              <div className="card-bg pump-border p-4 rounded-xl">
                <p className="text-[10px] text-gray-500 font-mono">测试网价格</p>
              ...
              <div className="card-bg pump-border p-4 rounded-xl">
                <p className="text-[10px] text-gray-500 font-mono">阶段</p>
                <p className={`font-bold uppercase ${isGraduated ? 'text-yellow-500' : 'text-blue-400'}`}>
                  {isGraduated ? '已毕业 (Uniswap)' : '联合曲线中'}
                </p>
              </div>
            </div>
            ...
            <div className="h-80 bg-black/40 rounded-2xl flex items-center justify-center border border-white/5 relative overflow-hidden group">
               ...
               <div className="text-center space-y-2 z-10">
                 ...
                 <p className="text-gray-500 font-mono text-sm tracking-widest uppercase mt-4">Base Sepolia 实时图表</p>
               </div>
            </div>
          </div>
          ...
          <div className="card-bg pump-border rounded-2xl p-6 md:p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold font-mono tracking-tight uppercase">发射进度</h2>
              <span className="text-green-500 font-mono text-sm font-bold bg-green-500/10 px-2 py-0.5 rounded">{progress?.toString() || '0'}%</span>
            </div>
            ...
            <div className="relative w-full h-10 bg-black rounded-full overflow-hidden border border-white/10 p-1 mb-6">
              ...
              <div className="absolute inset-0 flex items-center justify-center font-black text-sm uppercase mix-blend-difference">
                {isGraduated ? 'TOKEN GRADUATED' : `距离上线 DEX 还有: ${100 - (Number(progress) || 0)}%`}
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex justify-between text-[10px] mb-2 font-mono uppercase tracking-wider">
                  <label className="text-gray-400">{isBuyMode ? '支出测试币' : '卖出数量'}</label>
                  <span className="text-gray-500">
                    余额: {isBuyMode ? `${ethBalance ? parseFloat(formatEther(ethBalance.value)).toFixed(4) : '0'} Sepolia ETH` : `${balance ? parseFloat(formatEther(balance as bigint)).toLocaleString() : 0} ${symbol}`}
                  </span>
                </div>
                <div className="relative group">
                  <input 
                    ...
                    placeholder="0.00"
                    className="w-full bg-black border border-white/10 rounded-2xl px-5 py-5 text-2xl font-black focus:outline-none focus:border-green-500/50 transition-all"
                  />
              ...
              <button 
                ...
                onClick={handleTrade}
                className={`w-full py-5 rounded-2xl font-black text-lg uppercase shadow-2xl transform transition-all active:scale-[0.98] disabled:opacity-30 ${isWrongNetwork ? 'bg-yellow-500 text-black' : isBuyMode ? 'bg-green-500 text-black hover:bg-green-400' : 'bg-red-500 text-white hover:bg-red-400'}`}
              >
                {isWrongNetwork ? '切换到 Base Sepolia' : isPending || isConfirming ? '交易进行中...' : (isGraduated && !isBuyMode ? '已上线 UNISWAP' : `${isBuyMode ? '立即买入' : '确认卖出'}`)}
              </button>

              <p className="text-[10px] text-center text-gray-600 uppercase font-mono tracking-tighter">
                注意：这是测试网交易，不消耗真实 ETH。
              </p>
